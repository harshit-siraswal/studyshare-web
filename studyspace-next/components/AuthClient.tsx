...
const Auth = () => {
  ...
  const [selectedCollege, setSelectedCollege] = useState<Record<string, any>>({});
  ...
  useEffect(() => {
    ...
    try {
      const storedCollege = window.localStorage.getItem("selectedCollege");
      if (storedCollege) {
        setSelectedCollege(JSON.parse(storedCollege));
      }
    } catch {
      setSelectedCollege({});
    }
