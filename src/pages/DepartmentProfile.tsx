...
const DepartmentProfile = () => {
  ...
  const { user } = useAuth();
  const { selectedCollegeId, selectedCollege, isReadOnly } = useCollege();
  ...
  useEffect(() => {
    ...
    const loadDepartmentData = async () => {
      try {
        ...
        const [noticeResponse, followedResponse, followerResponse] =
          await Promise.all([
            api.getNotices({
        ...
        const activeNotices = (noticeResponse.notices || [])
          .filter(
            (notice) =>
              getEffectiveNoticeDepartment(notice) === departmentValue,
          )
          .filter(
            (notice) =>
        ...
        setIsFollowing(
          (followedResponse.departments || []).includes(departmentValue),
        );
                              ...
                              <div className="flex-1 flex gap-2">
                                <Input
                                  ...
                                  onKeyDown={(e) =>
                                    e.key === "Enter" &&
                                    !e.shiftKey &&
                                    handleReply(notice.id, newComment)
                                  }
                                />
                                <Button
                                  size="icon"
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      {comment.user_email === user?.email && (
                                        <Button
                                          ...
                                          onClick={() =>
                                            deleteComment(notice.id, comment.id)
                                          }
                                        >
