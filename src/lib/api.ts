...
export function isAiTokenQuotaExceededPayload(
  payload: any,
): payload is AiTokenQuotaErrorPayload {
  return payload?.error === "AiTokenQuotaExceeded";
}

export function formatAiTokenQuotaMessage(
  payload: AiTokenQuotaErrorPayload,
): string {
  ...
  const parts = [
    ...
    required > 0
      ? `Estimated required for this request: ${required} tokens.`
      : "",
  ].filter(Boolean);
  return parts.join(" ");
}
...
async function apiRequest<T>(
  ...
  const { timeoutMs = DEFAULT_API_REQUEST_TIMEOUT_MS, ...requestOptions } =
    options;
  ...
  const send = async (token: string | null) => {
    ...
    try {
      ...
      throw new ApiError(
        "Failed to reach server. Please check your connection and retry.",
        0,
        {
          error: "network_error",
          endpoint,
...
export async function unfollowUser(
  ...
  return apiRequest(`/api/follow/${encodeURIComponent(targetEmail)}`, {
    method: "DELETE",
  });
}
...
export async function checkFollowStatus(targetEmail: string): Promise<{
  ...
  const data = await apiRequest<any>(
    `/api/follow/status/${encodeURIComponent(targetEmail)}`,
  );
  return {
    status: data?.status,
...
export async function unfollowDepartment(
  ...
  return apiRequest(
    `/api/departments/follow/${departmentId}?collegeId=${encodeURIComponent(collegeId)}`,
    { method: "DELETE" },
  );
}
...
export async function getFollowedDepartments(
  ...
  return apiRequest(
    `/api/departments/following?collegeId=${encodeURIComponent(collegeId)}`,
  );
}
...
export async function getAcademicCatalog(
  ...
  return apiRequest(
    `/api/academics/catalog${queryString ? `?${queryString}` : ""}`,
  );
}

export async function resolveResourceScopes(selectedScope: {
  branch: string;
  semester: string;
  subject: string;
...
export async function getResourceVideoTopics(
  resourceId: string,
): Promise<VideoTopicResponse> {
 ...
 */
export async function getUserChatVotes(
  roomId: string,
): Promise<{ votes: Record<string, "up" | "down"> }> {
...
export async function getSavedChatPosts(): Promise<{
  savedPosts: SavedChatPost[];
}> {
  return apiRequest("/api/chat/saved");
}
...
function findNestedNumberByKeys(
  raw: unknown,
  keys: string[],
): number | undefined {
...
function normalizeUserProfile(raw: any): UserProfile {
  ...
  const subscriptionTierKeys = ["subscription_tier", "subscriptionTier"];
  ...
  const email =
    pickFirstTextFromSources(sources, ["email", "user_email", "userEmail"]) ||
    "";
  const displayName =
    pickFirstTextFromSources(sources, [
      "display_name",
      "displayName",
      "name",
  ...
  return {
    ...
    profile_photo_url: pickFirstTextFromSources(sources, [
      "profile_photo_url",
      "avatar_url",
      "photo_url",
      "avatarUrl",
    ...
    ai_token_budget:
      pickFirstNumberFromSources(tokenSources, budgetKeys) ??
      findNestedNumberByKeys(raw, budgetKeys),
    ai_token_used:
      pickFirstNumberFromSources(tokenSources, usedKeys) ??
      findNestedNumberByKeys(raw, usedKeys),
    ai_token_remaining:
      pickFirstNumberFromSources(tokenSources, remainingKeys) ??
      findNestedNumberByKeys(raw, remainingKeys),
    ai_token_base_budget:
      pickFirstNumberFromSources(tokenSources, baseBudgetKeys) ??
      findNestedNumberByKeys(raw, baseBudgetKeys),
    ai_token_budget_multiplier:
      pickFirstNumberFromSources(tokenSources, budgetMultiplierKeys) ??
      findNestedNumberByKeys(raw, budgetMultiplierKeys),
    ai_token_premium_multiplier:
      pickFirstNumberFromSources(tokenSources, premiumMultiplierKeys) ??
      findNestedNumberByKeys(raw, premiumMultiplierKeys),
    ai_token_cycle_days:
      pickFirstNumberFromSources(tokenSources, cycleDaysKeys) ??
      findNestedNumberByKeys(raw, cycleDaysKeys),
    ai_token_cycle_started_at: pickFirstTextFromSources(
      tokenSources,
      cycleStartedAtKeys,
    ),
    ai_token_cycle_ends_at: pickFirstTextFromSources(
      tokenSources,
      cycleEndsAtKeys,
    ),
    subscription_tier: pickFirstTextFromSources(sources, subscriptionTierKeys),
    subscription_end_date: pickFirstTextFromSources(
      sources,
      subscriptionEndDateKeys,
    ),
    ai_budget_inr:
      pickFirstNumberFromSources(tokenSources, budgetInrKeys) ??
...
export async function getMyProfile(): Promise<{ profile: UserProfile }> {
  ...
  if (!data) {
    throw lastError instanceof ApiError
      ? lastError
      : new ApiError("Failed to load profile", 0, {
...
export async function getAiTokenBalance(): Promise<AiTokenBalance> {
  ...
  const legacyEndpoints = String(
    import.meta.env.VITE_LEGACY_AI_BALANCE_ENDPOINTS ?? "",
  )
    .split(",")
...
export async function getNoticeComments(
  noticeId: string,
): Promise<{ comments: NoticeComment[] }> {
 ...
 */
export async function getNoticeLikes(
  noticeId: string,
