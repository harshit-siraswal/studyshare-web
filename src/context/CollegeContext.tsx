...
export const COLLEGES: College[] = [
  ...
  {
    id: "du-students",
    name: "Delhi University (Students)",
    domain: "students.du.ac.in",
  },
];
    ...
    if (stored) {
      ...
      if (parsed && parsed.domain) {
        ...
        if (matched)
          return {
            ...matched,
            collegeId: storedCollegeId || matched.collegeId || null,
          };
...
export const CollegeProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const loadCollegeIds = async () => {
      try {
        ...
        console.error(
          "[CollegeContext] Unexpected college ID lookup error:",
          error,
        );
      }
    };
  ...
  useEffect(() => {
    ...
    if (user?.email) {
      ...
      const matchedCollege = COLLEGES.find((c) =>
        domainMatches(userDomain, c.domain),
      );
      ...
      if (matchedCollege) {
        ...
        localStorage.setItem(
          "selectedCollege",
          JSON.stringify(hydratedCollege),
        );
        return;
      }
    }
  ...
  const hasElevatedRoleFallback = ["TEACHER", "ADMIN", "MODERATOR"].includes(
    normalizeDomain(user?.role).toUpperCase(),
  );
  ...
  const accessLevel: AccessLevel = (() => {
    if (hasElevatedAccess || hasElevatedRoleFallback) return "full";
